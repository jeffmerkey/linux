...
static int __init mdb_init_module(void)
{
	...
	if (!IS_ERR(filp)) {
		...
		if (size) {
			...
			pr_warn("MDB:  kgdb currently set to[%s], attempting to disable.\n", kdbstate);

			kdbstate[0] = '\0';
			size = mdb_kernel_write(filp, kdbstate, 1, 0);
			if (!size) {
				pr_warn("MDB:  kgdb/kdb active, MDB set to disabled. unload/reload to retry.\n");
				atomic_inc(&kgdb_detected);
			} else
				pr_warn("MDB:  kgdb/kdb set to disabled. MDB is enabled.\n");
		}
